...
model.SOLAR_GEN = Set()  # Set for solar generators
model.WIND_GEN = Set()  # Set for wind generators
...
model.Gen_isInSvc = Param(model.GEN, mutable=True , default = 1) # is this unit in service
...
model.Gen_energyRamp = Param (model.GEN, within=NonNegativeReals,default=0, mutable=True)  #  energy ramping rate between multiple periods
model.Gen_spinRamp = Param (model.GEN, within=NonNegativeReals,default=0, mutable=True)  # spin ramping rate for contingency
model.Gen_costCurveFlag = Param(model.GEN, default=0, mutable=True)
 # 1 indicates a cost curve is available
model.Gen_type = Param(model.GEN, mutable=True)
  # New parameter to classify generators
...
model.Solar_pg = Var(model.SOLAR_GEN, within=NonNegativeReals)
model.Wind_pg = Var(model.WIND_GEN, within=NonNegativeReals)
...
def const_SolarGenLimit(model, s):
    return model.Solar_pg[s] <= model.Solar_pgMax[s]
model.solarGenLimitConst = Constraint(model.SOLAR_GEN, rule=const_SolarGenLimit)
...
def const_NodeBal(model, n):
    ...
    expr += sum(model.Solar_pg[s] for s in model.SOLAR_GEN if model.Solar_busNumber[s] == n)
    expr += sum(model.Wind_pg[w] for w in model.WIND_GEN if model.Wind_busNumber[w] == n)
    expr -= sum(model.loadServed[d] for d in model.LOAD if model.Load_busNumber[d] == n)
    expr -= sum(model.pk[k] for k in model.BRANCH if model.Branch_frmBusNumber[k] == n)
    expr += sum(model.pk[k] for k in model.BRANCH if model.Branch_toBusNumber[k] == n)
    return expr == 0
model.nodeBalConst = Constraint(model.BUS, rule=const_NodeBal)


def const_GenMax(model, g):
    return model.pg[g] <= model.Gen_pgMax[g]

model.genMaxConst = Constraint(model.GEN, rule=const_GenMax)

def const_SolarMax(model, s):
    return model.Solar_pg[s] <= model.Solar_pgMax[s]

model.solarMaxConst = Constraint(model.SOLAR_GEN, rule=const_SolarMax)

def const_WindMax(model, w):
    return model.Wind_pg[w] <= model.Wind_pgMax[w]

model.windMaxConst = Constraint(model.WIND_GEN, rule=const_WindMax)
...
def const_FixOfflineAndNegativeLoad(model, d):
    if value(model.Load_isInSvc[d]) == 0:
...
def const_CalcFlow(model, k):
    if value(model.Branch_isInSvc[k]) == 0:
        return model.pk[k] == 0
    else:
...
def const_GenSegmentSum(model, g):
    if value(model.Gen_isInSvc[g]) == 0:
        return model.pg[g] == 0
...
def const_GenCostFixed(model, i):
    ...
    if value(model.Gen_isInSvc[genIdx]) == 0:
...
def const_GenCostCurveFlag(model, g):
    if value(model.Gen_costCurveFlag[g]) == 0:  # Use `value()` here
...
def const_InterfaceLimit(model, i):
    if value(model.Interface_isEnabled[i]) == 1:
...
def const_NodeBal_Ctgcy(model, c, n):
    if value(model.Contingency_isEnabled[c]) == 0:
        return Constraint.Skip
    else:
        expr = sum(model.pgc[g, c] for g in model.GEN if model.Gen_busNumber[g] == n)
        expr -= sum(model.loadServed_c[c, d] for d in model.LOAD if model.Load_busNumber[d] == n)
        expr -= sum(model.pkc[c, k] for k in model.BRANCH if model.Branch_frmBusNumber[k] == n)
        expr += sum(model.pkc[c, k] for k in model.BRANCH if model.Branch_toBusNumber[k] == n)
        return expr == 0
...
def const_LoadTotalLimit_Ctgcy(model, c, d):
    if value(model.Contingency_isEnabled[c]) == 0:
        ...
        expr -= model.Load_pd[d] * model.Load_isInSvc[d]
        return expr == 0
...
def const_GenMaxLimit_Ctgcy(model, c, g):
    if value(model.Contingency_isEnabled[c]) == 0:  # Use value() here
        return model.pgc[g, c] == 0
    if value(model.Gen_isInSvc[g]) == 0:  # Use value() here
        return model.pgc[g, c] == 0
    else:
...
def const_GenCostCurveFlag_Ctgcy(model, c, g):
    if value(model.Contingency_isEnabled[c]) == 0:  # Use `value()` for evaluation
        return Constraint.Skip
    elif value(model.Gen_costCurveFlag[g]) == 0:  # Use `value()` for evaluation
...
def const_SpinRampLimit_Ctgcy(model, c, g):
    if value(model.Contingency_isEnabled[c]) == 0:  # Use value() here
        return Constraint.Skip
    elif value(model.Gen_isInSvc[g]) == 0:  # Use value() here
        return Constraint.Skip
    else:
...
def const_InterfaceLimit_Ctgcy(model, c, i):
    if value(model.Contingency_isEnabled[c]) == 0:  # Use value() for evaluation
        return Constraint.Skip
    elif value(model.Interface_isEnabled[i]) == 1:  # Use value() for evaluation
...
def const_CalcInterfaceTotalFlow_Ctgcy(model, c, i):
    if value(model.Contingency_isEnabled[c]) == 0:  # Use value() for evaluation
        ...
        expr = 0
        for k in model.INTERFACELINE:
            if value(model.Interfaceline_interfaceIdx[k]) == i:  # Use value() for comparison
                expr += model.pkc[c, model.Interfaceline_branchIdx[k]]
...
def costObj(model):
    # Base costs
    conventional_cost = sum(model.pgi[i] * model.GenCost_segmentPrice[i] for i in model.GENCOST)
    solar_cost = sum(model.Solar_pg[s] * 2 for s in model.SOLAR_GEN)  # Assign a cost of 2/MWh
    wind_cost = sum(model.Wind_pg[w] * 1.5 for w in model.WIND_GEN)  # Assign a cost of 1.5/MWh
    
    # Penalties
    reserve_penalty = model.SR_penalty * sum(model.srReqSlackVar[g] for g in model.GEN)
    line_flow_penalty = model.BrcFlowLimit_penalty * sum(model.brcFlowLimitSlackVar[k] for k in model.BRANCH)
    load_shedding_penalty = model.LoadShed_penalty * sum(model.loadShed[d] for d in model.LOAD)
    ramp_up_penalty = model.energyRampUp_penalty * sum(model.energyRampUpSlackVar[g] for g in model.GEN)
    ramp_down_penalty = model.energyRampDown_penalty * sum(model.energyRampDownSlackVar[g] for g in model.GEN)
    interface_limit_penalty = model.InterfaceLimit_penalty * sum(model.interfaceLimiteSlackVar[k] for k in model.INTERFACE)
    contingency_penalty = model.LoadShed_c_penalty * sum(
        model.loadShed_c[c, d] for c in model.CONTINGENCY for d in model.LOAD
    )

    # Spinning reserves and slack variables
    spinning_reserves = model.SR_price * sum(model.sr[g] for g in model.GEN)
    spin_ramp_penalty = model.spinRamp_penalty * sum(model.spinRampSlackVar[g] for g in model.GEN)
